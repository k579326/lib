...
namespace fileutil {
    ...
    int File::Open(OpenModel om, AccessModel am, int permission)
    {
        ...
        CptString dir = pathutil::GetDirOfPathName(path_);
        if (!PathFileExists(dir.c_str())) {
            return -1;
        }

        BOOL exist = PathFileExists(path_.c_str());
        if (om == kCreate && exist)
            return -1;
        if (om == kOpen && !exist)
            return -1;
        ...
        file_->hFile_ = CreateFile( path_.c_str(),
    ...
    bool File::GetFileDescriptor(FileDescriptor* fd) const
    {
        ...
        fd->handle = file_->hFile_;
        return true;
    }

    void File::Reset(const CptChar* path)
    {
        path_ = path;
    ...
    // static 
    uint64_t File::Size(const CptString& filepath)
    {
        uint64_t size;
        if (filepath.empty()) {
            return 0;
        ...
        if (!GetFileAttributesEx(filepath.c_str(), GetFileExInfoStandard, &wfa))
        {
            return 0;
        }
    ...
    // static 
    int File::Remove(const CptString& filepath)
    {
        return DeleteFile(filepath.c_str()) == TRUE ? 0 : -1;
    }
    // static 
    int File::Copy(const CptString& srcpath, const CptString& dstpath, bool overlay)
    {
        return CopyFile(srcpath.c_str(), dstpath.c_str(), !overlay) == TRUE? 0:-1;
    }
    // static 
    int File::Truncate(const CptString& filename, uint64_t length)
    {
    ...
    // static 
    bool File::IsNormalFile(const CptString& filename)
    {
        DWORD attr = GetFileAttributes(filename.c_str());
        return attr & FILE_ATTRIBUTE_ARCHIVE;
    }
    // static 
    bool File::IsExist(const CptString& path)
    {
        return PathFileExists(path.c_str());
    }
    // static 
    int File::Rename(const CptString& oldname, const CptString& newname)
    {
        return MoveFile(oldname.c_str(), newname.c_str()) ? 0 : -1;
    }
};
